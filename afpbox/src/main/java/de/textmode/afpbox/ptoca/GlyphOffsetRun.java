...
/**
 * The Glyph Offset Run control sequence specifies an offset (relative displacement) from
 * the current baseline (in the b-direction) to the glyph origin for each glyph ID in
 * the preceding GIR.
 */
public final class GlyphOffsetRun extends PtocaControlSequence {

    /**
     * Constructs the {@link GlyphOffsetRun}.
     *
     ...
     */
    GlyphOffsetRun(final int functionType, final byte[] data) throws AfpException {
        super(functionType, data);

        if (this.getData().length < 4 || (this.getData().length & 0x01) == 1) {
            throw new AfpException(
                    "PTOCA control sequence GOR has invalid length of "
                            + this.getData().length + " bytes (expected at least 4 bytes, only even values)");
        }
    }

    /**
     * Returns the glyph offsets from the current baseline.
     *
     * @return glyph offsets from the current baseline.
     */
    public int[] getGlyphOffset() {
        ...
        for (int ix = 0, off = 4; ix < max; ++ix, off += 2) {
            result[ix] = ByteUtils.toInteger16(this.getData(), off);
        }
