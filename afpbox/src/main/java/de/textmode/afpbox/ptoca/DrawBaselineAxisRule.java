 ...
 */
public final class DrawBaselineAxisRule extends PtocaControlSequence {

    /**
     * Constructs the {@link DrawBaselineAxisRule}.
     *
     ...
     */
    DrawBaselineAxisRule(final int functionType, final byte[] data) throws AfpException {
        super(functionType, data);

        if (this.getData().length != 4 && this.getData().length != 7) {
            throw new AfpException("PTOCA control sequence DBR has invalid length of " + this.getData().length + " bytes (expected 4 or 7 bytes)");
        }
    }
